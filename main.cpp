...
public:
    ...
    void compareBooks() {
        ...
        for (const auto& [bookName, _] : normalizedScores) {
        ...
        for (size_t i = 0; i < bookList.size(); ++i) {
            for (size_t j = i + 1; j < bookList.size(); ++j) {
                ...
                unordered_set<string> wordsInBook1;
                for (const auto& [word, score] : normalizedScores[book1]) {
                    wordsInBook1.insert(word);
                }

                int matchCount = 0;
                for (const auto& [word, score] : normalizedScores[book2]) {
                    if (wordsInBook1.count(word)) {
                        matchCount++;
                    }
                }
